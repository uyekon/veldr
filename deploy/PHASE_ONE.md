...
白板 ID `dailyPush` 更名为 `dp`：读取旧数据时保留正文、版本、更新时间和历史快照，下一次成功写入 CMS 时将规范化后的 ID 写入磁盘。旧 API `/whiteboards/dailyPush` 仍作为 `dp` 的读写别名，并保留版本冲突保护；旧页面链接会转到 `#/whiteboard/dp`。如果数据库同时存在两个 ID，拒绝加载并要求人工核对，不静默丢弃其中一份。

**此 ID 迁移后的版本不能直接回滚到只认识 `dailyPush` 的旧后端。** 回滚前须停服备份，并将当前数据中的 `dp` 反向更名为 `dailyPush`（保留最新内容），或使用带该兼容逻辑的修复版本；不要用历史备份覆盖后续新内容。

标签导航按当前 Notebook 计算名称和数量；Docs 使用全局范围。普通标签只统计未归档文章，归档文章仅出现在 `archived` 标签筛选中，数量为 0 的标签不显示。
...
计划北京时间每天 03:00 运行。该 timer 已于 2026-09-17 在生产启用，并完成一次成功备份及清单校验；后续仍需监控每日执行结果。
...
- **待验收**：真实 iOS Safari / Android Chrome（含软键盘）和个人日常持续使用。生产 timer 已启用；原 JSON 备份以及阶段二 PostgreSQL 备份均已使用生产数据副本完成隔离恢复演练。
